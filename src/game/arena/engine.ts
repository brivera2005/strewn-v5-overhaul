...
import type { MetaSave, MetaUpgrades } from './types';
import type {
  ArenaState,
  CharmId,
  Enemy,
...
function floatText(state: ArenaState, x: number, y: number, text: string, color: string): void {
...
function addBurden(state: ArenaState, amount: number, upgrades: MetaUpgrades): void {
  ...
  if (conduitLvl > 0 && state.minions.length > 0) {
    ...
    for (const m of state.minions) {
      const share = route / state.minions.length;
      m.burdenHeld = Math.min(m.capacity, m.burdenHeld + share);
    }
...
export function createArenaState(meta: MetaSave): ArenaState {
  ...
  const state: ArenaState = {
    ...
    burden: { current: 0, max: BASE_BURDEN_MAX + u.burdenCap * 25 },
    enemies: [],
    projectiles: [],
    pickups: [],
    minions: [],
...
function killEnemy(state: ArenaState, enemy: Enemy, meta: MetaSave): void {
  ...
  addBurden(state, burdenGain, meta.upgrades);

  const emberRelay = charmLevel(state.charms, 'ember_relay');
  if (emberRelay > 0) {
    for (const e of state.enemies) {
      const d = dist(enemy.x, enemy.y, e.x, e.y);
      if (d < 80 + emberRelay * 20) {
        e.hp -= 8 * emberRelay;
        e.hitFlash = 0.1;
      }
    }
    spawnParticle(state, enemy.x, enemy.y, '#fb923c', 10, 160);
  }
  ...
  state.pickups.push({
    ...
    y: enemy.y,
    kind: 'xp',
    value: XP_GEM_VALUE + state.wave * 2,
    magnetized: false,
  });
  ...
  if (enemy.type === 'boss') {
    ...
    floatText(state, enemy.x, enemy.y, 'ANCHOR SHATTERED', COLORS.shard);
  }

  spawnParticle(state, enemy.x, enemy.y, COLORS.enemyWretch, 8);
  state.hitStop = 0.04;
  state.shake = 0.08;
}
...
export function tryMeld(state: ArenaState): CharmId | null {
  ...
  floatText(state, state.player.x, state.player.y - 30, `MELD: ${CHARM_DEFS[result].name}`, CHARM_DEFS[result].color);
  spawnParticle(state, state.player.x, state.player.y, CHARM_DEFS[result].color, 20, 200);
  state.shake = 0.2;
  return result;
}
...
export function deployMinion(state: ArenaState, meta: MetaSave): boolean {
  ...
  state.minions.push({
    ...
    maxHp: 50,
    burdenHeld: 0,
    capacity: MINION_CAPACITY + charmLevel(state.charms, 'spite_web') * 15,
  });
  spawnParticle(state, state.player.x, state.player.y, COLORS.minion, 8);
  return true;
}
...
export function updateArena(
  ...
  if (spongeLvl > 0) {
    drainBurden(state, 3 * spongeLvl * dt);
  }

  const voidLvl = charmLevel(state.charms, 'void') + charmLevel(state.charms, 'void_conduit');
  if (voidLvl > 0) {
  ...
  if (state.burden.current >= state.burden.max) {
    ...
    if (thornsLvl > 0) {
      for (const e of state.enemies) {
        ...
        if (d < 60 + thornsLvl * 15) {
          e.hp -= 12 * thornsLvl * dt;
        }
      }
    }
    ...
    if (burnLvl > 0 && Math.random() < dt * 2) {
      for (const e of state.enemies) {
        if (dist(state.player.x, state.player.y, e.x, e.y) < 100) {
          e.hp -= 20 * burnLvl;
          e.hitFlash = 0.12;
        }
      }
      spawnParticle(state, state.player.x, state.player.y, '#f97316', 4, 80);
    }
  }
  ...
  if (emberLvl > 0) {
    ...
    for (const e of state.enemies) {
      if (dist(state.player.x, state.player.y, e.x, e.y) < 55 + emberLvl * 10) {
        e.hp -= burnDmg * dt;
      }
    }
  }
  ...
  if (state.altarTimer <= 0 && !state.altarActive) {
    ...
    floatText(state, ARENA_W / 2, 80, 'MELD ALTAR ACTIVE', COLORS.altar);
  }
  ...
  if (state.waveTimer >= WAVE_DURATION || state.waveEnemiesLeft <= 0) {
    if (state.enemies.length === 0 && !state.bossActive) {
      state.wave += 1;
      state.waveTimer = 0;
      state.waveEnemiesLeft = 10 + state.wave * 3;
      if (state.wave % BOSS_WAVE_INTERVAL === 0) {
        state.bossActive = true;
        spawnEnemy(state, 'boss');
        floatText(state, ARENA_W / 2, 120, `GRIEF ANCHOR WAVE ${state.wave}`, COLORS.enemyBoss);
      } else {
  ...
  const spawnRate = 0.8 + state.wave * 0.15;
  if (state.waveEnemiesLeft > 0 && state.enemies.length < 30 + state.wave * 2) {
    if (Math.random() < spawnRate * dt) {
      const roll = Math.random();
      const type: Enemy['type'] =
        roll < 0.55 ? 'wretch' : roll < 0.8 ? 'howler' : 'anchor';
  ...
  for (const enemy of state.enemies) {
    ...
    enemy.x += ((state.player.x - enemy.x) / d) * enemy.speed * dt;
    enemy.y += ((state.player.y - enemy.y) / d) * enemy.speed * dt;
    if (d < PLAYER_RADIUS + enemy.size) {
      if (state.player.invuln <= 0) {
        state.player.hp -= enemy.damage * dt;
        state.damageTaken += enemy.damage * dt;
        addBurden(state, enemy.burdenEmit * 0.3 * dt, meta.upgrades);
      }
    }
    addBurden(state, enemy.burdenEmit * 0.05 * dt, meta.upgrades);
    if (enemy.hitFlash > 0) enemy.hitFlash -= dt;
  }

  if (!('_shardCd' in state)) (state as ArenaState & { _shardCd?: number })._shardCd = 0;
  const shardLvl = charmLevel(state.charms, 'shard') + charmLevel(state.charms, 'burden_storm');
  const stormLvl = charmLevel(state.charms, 'burden_storm');
  const shardCd = (state as ArenaState & { _shardCd: number })._shardCd;
  const autoFireInterval = Math.max(0.25, 0.9 - shardLvl * 0.1 - stormLvl * 0.08);
  ...
  if (shardLvl > 0 && shardCd + dt >= autoFireInterval) {
    ...
    const nearest = state.enemies
      .map((e) => ({ e, d: dist(state.player.x, state.player.y, e.x, e.y) }))
      .sort((a, b) => a.d - b.d)[0];
  ...
  for (const p of state.projectiles) {
    ...
    for (const enemy of state.enemies) {
      if (dist(p.x, p.y, enemy.x, enemy.y) < enemy.size + 4) {
        enemy.hp -= p.damage;
        enemy.hitFlash = 0.08;
        spawnParticle(state, p.x, p.y, p.color, 3, 60);
        if (p.pierce > 0) {
          p.pierce -= 1;
        } else {
          p.life = 0;
  ...
  for (const e of deadEnemies) killEnemy(state, e, meta);
  ...
  for (const m of state.minions) {
    ...
    if (voidConduit > 0 && m.burdenHeld > 5) {
      const target = state.enemies[0];
      if (target) {
        fireProjectile(state, m.x, m.y, target.x, target.y, 8 * voidConduit, '#a5b4fc');
        m.burdenHeld -= 3 * dt;
      }
    }
  }
  ...
  for (const pick of state.pickups) {
    ...
    if (d < 120) pick.magnetized = true;
    if (pick.magnetized) {
      const spd = 280;
      const dd = d || 1;
      pick.x += ((state.player.x - pick.x) / dd) * spd * dt;
      pick.y += ((state.player.y - pick.y) / dd) * spd * dt;
    }
    if (d < PLAYER_RADIUS + 8) {
      if (pick.kind === 'xp') gainXp(state, pick.value, meta);
      else if (pick.kind === 'shard') {
        /* counted at run end */
      }
      spawnParticle(state, pick.x, pick.y, pick.kind === 'xp' ? COLORS.xp : COLORS.shard, 4);
      continue;
    }
...
export function getRunStats(state: ArenaState, shardBonus: number): RunStats {
  ...
  const baseShards = Math.floor(state.kills * 0.3 + state.wave * 5 + state.meldsFound * 15);
  const shardsEarned = Math.floor(baseShards * (1 + shardBonus * 0.15)) + shardPickup;
  return {
    time: state.time,
    kills: state.kills,
    wavesCleared: state.wave - 1,
    levelsGained: state.player.level - 1,
    meldsFound: state.meldsFound,
    shardsEarned,
    damageTaken: state.damageTaken,
    burdenOverflows: state.burdenOverflows,
  };
}
