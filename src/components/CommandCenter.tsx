...
export function CommandCenter({ state, onSelectPatient, onAlertClick }: Props) {
  ...
  const metrics = [
    { label: 'Active cases', value: active, tip: 'Living cases in triage' },
    { label: 'Critical', value: critical, tip: 'Cases needing immediate deployment' },
    { label: 'Stable', value: stable, tip: 'Cases with manageable pain load' },
    ...
    { label: 'Stabilized', value: state.triageStats.stabilized, tip: 'Cases brought to stable status' },
    { label: 'Draft threshold', value: state.triageStats.draftThreshold, tip: 'Risk of forced minion draft' },
    { label: 'Minion pool', value: state.participantPoolSize, tip: 'Total minion capacity' },
    { label: 'Relics', value: state.inventory.length, tip: 'Loot items equipped' },
    ...
    { label: 'Memorial', value: state.triageStats.memorialCount, tip: 'Cases lost to memorial' },
    { label: 'Avg pain', value: Math.round(painHistory.reduce((a, b) => a + b, 0) / Math.max(1, painHistory.length)), tip: 'Average pain load' },
      ...
      <div className="command-grid">
        <div className="command-metrics glass-panel">
          ...
          <div className="gauge-row">
            ...
            <Tooltip content="Minions deployed vs available capacity">
              <div className="gauge-widget">
                <label>Minions {network.allocated}/{network.total}</label>
        ...
        <div className="command-alerts glass-panel">
          ...
          <div className="alerts-feed">
            {recentAlerts.length === 0 && (
              <p className="alerts-empty">No alerts yet. Keep deploying minions.</p>
            )}
            {recentAlerts.map((a) => (
              <button
                key={a.id}
            ...
            {alerts.critical > 0 && (
              <button type="button" className="alert-item severity-critical" onClick={() => onAlertClick()}>
                {alerts.critical} case(s) in critical state
              </button>
            )}
          </div>
        </div>
